    ...
    switch (materialType) {
        case MAT_TYPE_LAMBERTIAN:
            if (materialIndex >= 0 && materialIndex < pc.lambertianMaterialCount) {
                ...
                vec3 albedo = getMaterialPropertyValue(lambertianMaterial.albedo, rec.meshVertex);
        ...
        case MAT_TYPE_METAL:
            if (materialIndex >= 0 && materialIndex < pc.metalMaterialCount) {
                ...
                vec3 albedo = getMaterialPropertyValue(metalMaterial.albedo, rec.meshVertex);
                vec3 fuzz = getMaterialPropertyValue(metalMaterial.fuzz, rec.meshVertex);
