...
layout(set = 10, binding = 0, scalar) buffer ConstantMedia {
...
layout(push_constant) uniform PushConstants {
    ...
    layout(offset = 60) uint  isotropicMaterialCount;
    layout(offset = 64) uint  lightSourceTriangleCount;
    layout(offset = 68) float lightSourceTotalArea;
    layout(offset = 72) uint  constantMediaCount;
} pc;
...
Material unpackMeshMaterial(const uint meshId) {
    Mesh mesh = meshData.values[meshId];
    return Material(mesh.materialType, mesh.materialIndex);
}

MeshTriangle unpackMeshVertex(const uint meshId, const uint primitiveId) {
...
uint unpackMeshType(const uint meshId) {
    Mesh mesh = meshData.values[meshId];
...
LightSample sampleLightSources(inout uint rngState, mat4x3 objectToWorld) {
    ...
    MeshTriangle light = unpackMeshVertex(meshId, primitiveId);
...
ScatterRecord isotropicMaterialScatter(inout uint rngState, uint materialIndex, HitRecord rec, float time) {
    ...
    if (materialIndex >= 0 && materialIndex < pc.isotropicMaterialCount) {
        ...
        vec3 attenuation = getMaterialPropertyValue(material.phaseFunction, rec.meshVertex);
...
void calculateSurfaceRecs(inout uint rngState, RayPayload rp, Ray ray, out HitRecord rec, out EmissionRecord erec, out ScatterRecord srec) {
    MeshTriangle hitTriangle = unpackMeshVertex(rp.meshId, rp.primitiveId);
    Material material = unpackMeshMaterial(rp.meshId);

    rec = getIntersection(hitTriangle, rp.surfaceHitAttribs, rp.objectToWorld, rp.worldToObject, ray.direction);

    erec = calculateEmission(rngState, material, rec);
    srec = calculateScatter(rngState, material, rec, ray.direction, rp.time);

    if (srec.isScattered && !srec.skipPdf) {
        // Get a the light source sample.
        LightSample lightSample = sampleLightSources(rngState, rp.objectToWorld);

        // Choose between material and light PDF with a 50-50 chance.
        uint chosenPdfType = chooseMixturePdf(rngState, srec.matPdfType);
        srec.scatterDirection = genScatterDirection(rngState, chosenPdfType, rec, rp.objectToWorld, lightSample);

        // Use material PDFs.
        float scatteringPdf = getPdfValue(srec.matPdfType, srec.scatterDirection, rec, lightSample);
        float pdfMat        = scatteringPdf;
        float pdfValue      = pdfMat;

        // See if we want to use a Mixture PDF.
        if (pc.lightSourceTriangleCount > 0 && pc.lightSourceTotalArea > 0.0) {
            float pdfLight = getPdfValue(LIGHT_PDF, srec.scatterDirection, rec, lightSample);
            pdfValue = 0.5 * pdfLight + 0.5 * pdfMat;
        }

        srec.attenuation *= scatteringPdf / pdfValue;
    }
}

// NOTE: Assumption - Volumes do not contain anything within. rp contains volume entry payload.
bool calculateVolumeRecs(inout uint rngState, RayPayload rp, Ray ray, out HitRecord rec, out EmissionRecord erec, out ScatterRecord srec) {
    /*
    Mesh mesh = meshData.values[rp.meshId];

    switch (mesh.mediumType) {
        case CONSTANT_MEDIUM:
            if (mesh.mediumIndex >= 0 && mesh.mediumIndex < pc.constantMediaCount) {
                ConstantMedium medium = constantMediumData.values[mesh.mediumIndex];
                Material material = Material(medium.materialType, medium.materialIndex);

                float rayLength = length(rd);
                float distanceInside = (tExit - tEntry) * rayLength;

                float hitDistance = -log(randomFloat(rngState)) / medium.density;

                if (hitDistance < distanceInside) {
                    // Compute hit in object space.
                    float t = tEntry + hitDistance / rayLength;
                    vec3  p = ro + t * rd;

                    // Convert to world space.
                    p = vec3(rp.objectToWorld * vec4(p, 1.0));

                    // Only hit point is used for volume scattering. Everything else ignored for now.
                    rec = HitRecord(MeshVertex(p, 0, vec3(0), 0), false, vec3(0));
                    srec = calculateScatter(rngState, material, rec, ray.direction, rp.time);
...
vec3 rayColour(inout uint rngState, Ray ray, uint rayFlags) {
    vec3 accumulated = vec3(0.0);
    vec3 throughput  = vec3(1.0);

    uint currentVolumeIndex = -1; // Invalid (we could be inside a medium though).
    ...
    for (uint depth = pc.maxRayDepth; depth > 0; --depth) {
        ...
        traceRayEXT(
                ...
                RAY_EPS,       // ray min range
                ray.direction, // ray direction
                RAY_INF,       // ray max range
        ...
        if (!rp.isMissed) {
            uint meshType = unpackMeshType(rp.meshId);

            if (meshType == SURFACE) {
                calculateSurfaceRecs(rngState, rp, ray, rec, erec, srec);
            } else if (meshType == VOLUME) {
                /*
                if (!rp.volumeEntering || !calculateVolumeRecs(rngState, rp, ray, rec, erec, srec)) {
                    // Ray left the volume or there was no scattering inside volume.
                    // Figure out the exit point on volume boundary to continue testing for intersections.
                    vec3 rd = vec3(rp.worldToObject * vec4(ray.direction, 1.0));
                    vec3 ro = vec3(rp.worldToObject * vec4(ray.origin, 1.0));
                    ro += rp.t * rd;
                    ray.origin = vec3(rp.objectToWorld * vec4(ro, 1.0));

                    // Don't count towards ray depth.
                    depth++;
                    continue;
                }
                */
        ...
        ray = Ray(rec.meshVertex.p, normalize(srec.scatterDirection), ray.time);
    }
...
void main() {
    ...
    for (int sj = 0; sj < sqrtSpp; ++sj) {
        for (int si = 0; si < sqrtSpp; ++si) {
            ...
            vec3 attenuation = rayColour(rngState, ray, gl_RayFlagsNoneEXT);
